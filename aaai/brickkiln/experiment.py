...
def run_experiment(config, device="cuda:0", epochs=50, batch_size=32, patience=3, eval_every=25):
    """
    Trains and evaluates a model as per experiment config.
    Evaluates every `eval_every` epochs, supports early stopping.
    """
    ...
    # 1. Build dataset and train model
    train_dataset = build_concat_dataset(
        config["train_images"], config["train_labels"], transforms=get_transform()
    )
    model, losses_per_epoch = train_model(
        train_dataset,
        device,
        num_classes=4,
        epochs=epochs,
    ...
    # 3. Evaluate every `eval_every` epochs
    for epoch in range(eval_every, epochs + 1, eval_every):
    ...
    # If last epoch wasn't evaluated (e.g., epochs not divisible by eval_every), evaluate at final epoch
    if (epochs % eval_every) != 0 and (log_df['epoch'].iloc[-1] != epochs):
        pred_dir = os.path.join(output_root, f"predictions_epoch_{epochs}")
        predict(model, test_images, get_transform(), pred_dir, device=device)
        map50 = compute_map(test_images, test_labels, pred_dir)
        print(f"[Epoch {epochs}] mAP@0.50: {map50:.4f}")
        log_df.loc[len(log_df)] = [epochs, map50]
        log_df.to_csv(csv_path, index=False)
        if map50 > best_map50:
            best_map50 = map50
            best_epoch = epochs
            model_path = os.path.join(output_root, "best_faster_rcnn.pth")
            torch.save(model.state_dict(), model_path)
            print(f"New best model saved at final epoch {epochs} with mAP@0.50 = {map50:.4f}")
